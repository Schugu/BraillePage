...
export default function Signo({ cadenaParaCodificar, formato }: SignoProps) {
  ...
  const elementosBraille = cadenaSeparada.map(caracter => {
    if (caracter in letrasBraille) {
      return (
        <article key={caracter} className={`grid grid-rows-3 grid-cols-2 place-items-center cursor-grab
        ...
        `}>
          {
            letrasBraille[caracter].map((punto, puntoIndex) => (
              <span key={puntoIndex}
                className={`${punto === 1 ? '' : 'bg-opacity-10'} bg-black rounded-full cursor-grab shadow-braille
